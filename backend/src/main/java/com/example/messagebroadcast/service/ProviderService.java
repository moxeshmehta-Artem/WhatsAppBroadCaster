...
public class ProviderService {
    ...
    public WhatsAppProvider updateStatus(Long id, String status) {
        ...
        // --- SECURITY GUARD ---
        // If the user attempts to deactivate a provider, we must ensure another one is still ACTIVE.
        if (newStatusEnum == ProviderStatus.INACTIVE && provider.getStatus() == ProviderStatus.ACTIVE) {
            ...
            if (activeCount <= 1) {
                throw new IllegalArgumentException("REJECTED: You cannot deactivate the last active provider. At-least one must remain active.");
            }
        }
